...
// ── fetchWithProxy ────────────────────────────────────────────────────────────
// Required by spec: single function that prepends the CORS proxy.
// Uses the first proxy in CONFIG.PROXIES (corsproxy.io by default).
// Falls back to RequestManager for retry + rotation if the first attempt fails.
async function fetchWithProxy(url) {
  // Try the primary proxy first (fastest path)
  const proxied = CONFIG.PROXIES[1](url); // index 1 = corsproxy.io
  try {
    const resp = await fetch(proxied, { mode: 'cors', credentials: 'omit' });
    if (resp.ok) return resp;
  } catch (_) {}
  // Fall back to RequestManager's full retry+rotation logic
